...
describe('FSI Server Client interface test', () => {
  ...
  it('deleteMetaData', async () => {
    const result = await client.deleteMetaData(basePath + "test/files/ü ö ä/" + myFiles[1],
      {iptc: {"Copyright Notice": "foo bar"}}, "file", opts);
    expect(result).to.equal(true);
  });
  ...
  it('renameFile()', async () => {
    const result = await client.renameFile(basePath + "test_move/files/ü ö ä/a.jpg", basePath +
      "test_move/files/ü ö ä/abc.jpg", opts);
    expect(result).to.equal(true);
  });
