...
import {ILoginReply, Login} from "./Login";
...
import {IUploadOptions, Upload} from "./Upload";
...
export default class FSIServerClient {
  ...
  public static DEFAULT_PROGRESS_FUNCTION(prg: QueueProgress | TaskProgress): void {
    ...
    try {
      ...
      if (qPrg.task) {
        ...
        msg += qPrg.currentTask.getMessage() + " - task progress: " + qPrg.task.percent.toFixed(1)
          + "% progress: ";
  ...
  public getAxiosRequestConfigUserAgent(options?: IHTTPOptions, headers?: IStringAnyMap): AxiosRequestConfig {
    ...
    if (modeNode) {
      if (!headers) headers = {};
      headers['User-Agent'] = FSIServerClientUtils.USERAGENT;
    }

    return this.getAxiosRequestConfig(options, headers);
  }
